 ...
 *
 * Inputs: 
 * int argc - the number of arguements passed in from the command line
 * char *argv - the list of arguements passed in from the command line
 * --------------------------------------------------------------------
 * This function initialize memory blocks that will be used for the 
 * duration of the program. It then calls getParams to parse the 
 * command line arguements into the allocated memory before 
 * initializing the MPI environment.
 * 
 * After MPI is initialize, a timer is started before the processes  
 * begin their Julia set calculations. When each process finishes, the 
 * timer is stopped and the statistics are collected on process 0 for 
 * output to the console. Process 0 is also responsible for converting
 *  the iterations calculated by Julia and converting them into .bmp 
 * files.
*/
