 ...
 * int maxIterations - maximum number of hops to try to exit the unit 
 *                     circle
 * int *iterations - the memory block that julia is working on
 * 
 * Outputs: 
 * long int iterationCount - the maximum number of iterations required 
 *                           by any pixel in the memory block
 * --------------------------------------------------------------------
 * This function generates iteration values for pixels in an image of 
 * resolution xres x yres. The memory block iterations is a xblock x 
 * yblock section of the complete image. The values startx and starty 
 * tell julia where the block is located in the image.
 * Julia tried maxIterations times to leave the unit circle for each 
 * pixel and records that value in iterations. The memory block 
 * iterations does not need to be explicitly returned because it is 
 * passed by reference.
*/
...
long int julia(mpf_t xmin, mpf_t xmax, int xblock, unsigned long int xres, int startx, mpf_t ymin, mpf_t ymax, int yblock, unsigned long int yres, int starty, mpf_t cr, mpf_t ci, int flag, int maxIterations, int *iterations)
{
  ...
  { 
	for (i = 0; i < xblock; i++)    
	{
	  ...
	  mpf_mul_ui(tempReal, xgap, (i+startx));  
	  mpf_add(zinitReal, xmin, tempReal);
	  
	  // zinitImag = y[0] + (j + starty) * ygap;
	  mpf_mul_ui(tempImag, ygap, (j+starty));  
	  ...
	  while (compare < 0 && iteration < maxIterations)
	  {
		...
		/* Real part - a^2 - b^2 */
		// tempReal = (zReal*zReal) - (zImag*zImag);
		mpf_mul(tempReal, zReal, zReal);       
		mpf_mul(tempImag, zImag, zImag);
		mpf_sub(tempReal, tempReal, tempImag);
			  
		/* Imaginary part - 2ab */
		// tempImag = 2 * zReal * zImag;
		mpf_mul_ui(tempImag, zReal, 2);         
		mpf_mul(tempImag, tempImag, zImag);
		
		// zReal = tempReal + z0Real;
		mpf_add(zReal, tempReal, z0Real);      
		// zImag = tempImag + z0Imag;
		mpf_add(zImag, tempImag, z0Imag);      
