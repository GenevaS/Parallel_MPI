 ...
 * 
 * Outputs: 
 * int totalCount - the total number of iterations required by 
 *                  all pixels in the process
 * --------------------------------------------------------------------
 * This function designates process 0 as a master process. It is 
 * responsible for allocating work to other processes and consolidating 
 * the work each slave process does when it returns to process 0. 
 * Process 0 does not compute values for the image itself. When there  
 * are no more rows left to compute in the image, the Master sends each  
 * slave process a DONE signal and exits.
 * 
 * All other processes are slaves. They are assigned work on a  
 * row-by-row basis. Slaves receive rows as indexes and compute the  
 * Julia set for that row. It then passes back the row and waits for 
 * the next message from the Master. If there are more rows, the Master 
 * sends a new row index. If there are no rows left, the Master sends a
 * DONE message and the slave process exits.
*/
