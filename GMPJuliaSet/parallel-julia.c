 ...
 *
 * Outputs: 
 * int maxCount - the maximum number of iterations required by any 
 *                pixel in the process
 * --------------------------------------------------------------------
 * This function determines which Julia computation algorithm to use  
 * based on the number of processes:
 *  - # Processes = 1: Serial program; call julia function directly
 *  - # Processes = 2: Not enough processes to require a task master;  
 *                     send to BlockPartitionJulia
 *  - # Processes > 2: Enough processes to require a task master; send 
 *                     to TaskMasterJulia
*/
