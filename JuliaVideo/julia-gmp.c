/*
 * Geneva Smith
 * JuliaVideo - julia-gmp.c
 * -------------------------------------------------------------------------------------------------
 * Function: julia
 * Inputs: mpf_t xmin, xmax - x coordinates to the left and right of the center point
 ...
 *         mpf_t ymin, ymax - y coordinates above and below the center point
 ...
 * starty tell julia where the block is located in the image. To generate a complete image with a
 * single function call, pass in startx and starty as 0, xblock as xres, and yblock as yres.
 *
 * Julia tries maxIterations times to leave the unit circle for each pixel and records that value in
 * iterations. The memory block referenced by iterations does not need to be explicitly returned  
